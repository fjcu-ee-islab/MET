...
def patch_visualization(All_pix_t, patch_image_counter, ts_counter):
    ...
    patch_mask_visual = ("./visual/Patch_visualization/patch_mask_visual/") 
    if os.path.exists(patch_mask_visual) == False: os.mkdir(patch_mask_visual)
    if len(os.listdir(patch_mask_visual)) != 0: 
        print()
        print()
        print("The direction ./visual/Patch_visualization/patch_mask_visual/ is not empty.")
        print("It will cause error when visualization.")
        print()
        breakpoint()
    ...
    for i in range(len(All_pix_t)):
        ...
        savemat('./visual/Patch_visualization/patch_mask_visual/patch_'+ str(patch_image_counter)+ '_ts_' + str(i)+'.mat', patch)
        ...
        plt_image_filename = './visual/Patch_visualization/patch_mask_visual/patch_'+ str(patch_image_counter)+ '_ts_' + str(i)+'.png'
...
def get_evaluation_results(path_model, path_weights, skip_validation=False, force=False, device='cpu'):
    ...
    if not skip_validation:
        ...
        #for polarity, pixels, labels in tqdm(dl):
            ...
            heatmap_dir = ("./visual/Patch_visualization/patch_heatmap/") 
            if os.path.exists(heatmap_dir) == False: os.mkdir(heatmap_dir)
            if len(os.listdir(heatmap_dir)) != 0: 
                print()
                print()
                print("The direction ./visual/Patch_visualization/patch_heatmap/ is not empty.")
                print("It will cause error when visualization.")
                print()
                breakpoint()
            ...
            for ts in range(len(polarity)):
                ...
                print("Saving gradcam weight in: ./visual/Patch_visualization/patch_heatmap ")
