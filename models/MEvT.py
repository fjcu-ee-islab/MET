...
class self_att_Block(nn.Module):
    ...
    def forward(self, z, z2, mask=None, q_mask=None, **args):
...
def get_block(name, params):
    ...
    elif name == 'self_att_Block': return self_att_Block(**params)
...
class EvNetBackbone(nn.Module):
    def __init__(self, 
        ...
        # Eden: Self-attention of latent memory 
        memory_self_att['params']['opt_dim'] = embed_dim
        memory_self_att['params']['embed_dim'] = embed_dim
        self.memory_self_att_blocks = nn.ModuleList([ get_block(memory_self_att['name'], memory_self_att['params']) ])
    ...
    def forward(self, kv, pixels):        
        ...
        # Initial latent vectors
        self_att_latent_vectors = self.memory_vertical.unsqueeze(1)
        self_att_latent_vectors = self_att_latent_vectors.expand(-1, batch_size, -1)    # (num_latent_vectors, batch_size, embed_dim)
        
        # Initialize self_att_q
        self_att_q = latent_vectors
        
        memory_self_att_block = self.memory_self_att_blocks[0]
        self_att_latent_vectors = memory_self_att_block(self_att_q, self_att_q)
        
        embs2 = self_att_latent_vectors
        
        ### Edit from Eden ###                
        
        res = self.proc_embs_block(embs2)      
        return res
