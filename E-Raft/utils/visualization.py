...
class DsecFlowVisualizer(BaseVisualizer):
    ...
    def visualize_events(self, image, batch, batch_idx, sequence_name):
        ...
        events_unrectified = numpy.stack([t, x, y, p], axis=-1)
        event_image = events_to_event_image(
            event_sequence=events_unrectified,
            height=h,
            width=w
        ).numpy()
...
def visualize_optical_flow(flow, savepath=None, return_image=False, text=None, scaling=None):
    ...
    savefile_name = savepath.split('/')[-1].split('.')[0] + '.h5'
    savefile = savepath.split('/')[0]+'/'+savepath.split('/')[1]+'/'+savepath.split('/')[2]+'/'+savepath.split('/')[3]+'/'+ savefile_name
    file_index = savepath.split('_')[-2]
    
    with h5py.File(savefile, 'w') as hf:
        flow_group = hf.create_group('flow')
        flow_group.create_dataset('u', data=flow[...,0])
        flow_group.create_dataset('v', data=flow[...,1])
        hf.create_dataset('file_index', data=file_index)
