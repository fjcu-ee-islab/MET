...
path_classes = PATH + '/AEDAT4.0/'

files = os.listdir(path_classes +'/')
parser = aermanager.parsers.parse_aedat4
path_aedat = path_classes +'/'
'''
...
for events_file in tqdm(files):
    ...
    file_name = events_file[:-7]
    #if file_name != 'f_houjing_2.1': continue
    
    shape, all_events = load_events_from_file(path_aedat + events_file, parser=parser)
    
    output_dir = PATH + '/OF_input/'
    if not os.path.isdir(output_dir):
        os.mkdir(output_dir)  
    output_class_dir = PATH + '/OF_input/' + '/test/'
    if not os.path.isdir(output_class_dir):
        os.mkdir(output_class_dir)        
    path_h5_dir = output_class_dir + file_name
    ...
    print("path_h5_file = ", path_h5_file)
    f = h5py.File(path_h5_file, 'w')

    t_offset_value = all_events['t'].min() #-1000000000 #uint64
    t_offset = f.create_dataset('t_offset', data = 0) #data = t_offset_value
    new_t = all_events['t'] - t_offset_value
    all_events['t'] = new_t
    #breakpoint()
