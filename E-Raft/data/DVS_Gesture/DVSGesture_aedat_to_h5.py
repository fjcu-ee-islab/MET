...
def find_closest_ts_every_n(events, path_image_timestamps_file, t):
    ...
    while(target_time <= end_time):        
        ...
        if img_ts == []:
            img_ts.append((min_t).astype('int').astype('str'))
            img_ts_file.write((min_t).astype('int').astype('str')+'\n')
        elif (t[t_idx]-n>t[t_idx-1]): 
            target_idx += 1
            target_time = min_t + target_idx * n
            print('target_idx = ', target_idx)
            continue
        else:
            if (t[t_idx]).astype('int').astype('str') != img_ts[-1]:     
                img_ts.append((t[t_idx]).astype('int').astype('str'))
                img_ts_file.write((t[t_idx]).astype('int').astype('str')+'\n')
            else: pass
...
parser = aermanager.parsers.parse_dvs_ibm
...
for events_file in tqdm(aedat_files):
    #if events_file != 'user21_natural.aedat': continue
    ...
    t_offset_value = all_events['t'].min()
    ...
    img_ts = find_closest_ts_every_n(events, path_image_timestamps_file, all_events['t'])
