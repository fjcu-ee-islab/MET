...
classes = os.listdir(path_classes)

for c in classes:
    
    files = os.listdir(path_classes + c +'/')
    parser = aermanager.parsers.parse_aedat4
    path_aedat = path_classes + c +'/'
    '''
    ...
    for events_file in tqdm(files):
        ...
        output_dir = PATH + '/OF_input/' + c
        if not os.path.isdir(output_dir):
            os.mkdir(output_dir)  
        output_class_dir = PATH + '/OF_input/' + c + '/test/'
