...
class EventDataset(Dataset):
    def __init__(self, samples_folder,  # Folder from where to load the data
        ...
        if dataset_name in ['DVS128', 'DVS128_SME', 'DVS128_OF', 'DVS128_SME_no_aug']:
        ...
        elif dataset_name in ['ASL_DVS', 'HMDB', 'UCF101', 'UCF50', 'SLAnimals_4s', 'SLAnimals_3s', 'SLAnimals_SME_3s', 'SLAnimals_SME_4s',  'SLAnimals_SME_3s_no_aug', 'SLAnimals_SME_4s_no_aug', 'SLAnimals_4s_no_aug', 'SLAnimals_4s_OF_no_aug', 'SLAnimals_OF_3s', 'SLAnimals_OF_4s', 'N_Cars', 'Caltech', 'fall_detection_dataset', 'FDD_SME', 'FDD_OF', 'ucf11', 'ucf11_SME', 'ucf11_OF', 'IITM', 'IITM_SME', 'IITM_OF']:
    ...
    def __getitem__(self, idx, return_sparse_array=False):
        ...
        if filename[:3]=='dir':
            #total_direction
            total_events = pickle.load(open(os.path.join(self.samples_folder + filename), 'rb'))
            data_name = 'dir'
        elif filename[:5]=='speed':
            #total_speed
            total_events = pickle.load(open(os.path.join(self.samples_folder + filename), 'rb'))
            data_name = 'sp'
        else:  
        ...
        if self.validation:
            total_events_chunk_to_visual_path = "./visual/Patch_visualization/total_events_chunk_to_visual.pckl"
            print("")
            print("")
            print("*"*40)
            print("If heatmap visualization is needed, please uncomment:  data_generation.py Line. 302")
            print("pickle.dump(total_events_chunk_to_visual, open(total_events_chunk_to_visual_path, 'wb'))")
            print("total_events_chunk_to_visual: ", total_events_chunk_to_visual_path)
            print("*"*40)
            print("")


        total_pixels, total_polarity = [], []
        total_events_chunk_to_visual = []
        current_chunk = None
        ...
        while sf_num >= 0:
            ...
            if current_chunk is None: 
                ...
                total_events_chunk_to_visual.append(current_chunk)
            ...
            if self.validation:
                # print("*** data_generation.py: sf_num + self.chunk_size = ", sf_num + self.chunk_size)
                # print("*** data_generation.py: sf_num = ", sf_num)
                # print("*** data_generation.py: polarity.shape = ", polarity.shape)
                # print("*** data_generation.py: pixels.shape = ", pixels.shape)
                total_events_chunk_to_visual_path = "./visual/Patch_visualization/total_events_chunk_to_visual.pckl"
                #pickle.dump(total_events_chunk_to_visual, open(total_events_chunk_to_visual_path, 'wb'))                
                # print("")
        ...
        return total_polarity, total_pixels, label, data_name
...
class Event_DataModule(LightningDataModule):
    def __init__(self, batch_size, chunk_len_ms, 
        ...
        if dataset_name == 'DVS128':
        ...
        elif dataset_name == 'DVS128_SME':
            self.data_folder = './datasets/DvsGesture_VM/clean_dataset_frames_24000/'
            self.width, self.height = 128, 128
            self.num_classes = 12 - len(classes_to_exclude)
            self.class_mapping = copy.deepcopy(DVS128_class_mapping)
            for c in classes_to_exclude: del self.class_mapping[c]
            self.class_mapping = { i:l[1] for i,l in enumerate(sorted(self.class_mapping.items(), key=lambda x:x[0])) }     
        elif dataset_name == 'DVS128_OF':
            self.data_folder = './datasets/DvsGesture_OF/clean_dataset_frames_24000/'
            self.width, self.height = 128, 128
            self.num_classes = 12 - len(classes_to_exclude)
            self.class_mapping = copy.deepcopy(DVS128_class_mapping)
            for c in classes_to_exclude: del self.class_mapping[c]
            ...
            self.data_folder = './datasets//SL_Animals/SL_animal_splits/dataset_3sets_12000/'
            self.width, self.height = 128, 128
            self.num_classes = 19
            self.class_mapping = { i:l for i,l in enumerate(range(self.num_classes)) }
        elif dataset_name == 'SLAnimals_4s':
            self.data_folder = './datasets//SL_Animals/SL_animal_splits/dataset_4sets_12000/'
            self.width, self.height = 128, 128
            self.num_classes = 19
            self.class_mapping = { i:l for i,l in enumerate(range(self.num_classes)) }
        elif dataset_name == 'SLAnimals_SME_3s':
            self.data_folder = './datasets//SL_Animals_VM/SL_animal_splits/dataset_3sets_48000/'
            self.width, self.height = 128, 128
            self.num_classes = 19
            self.class_mapping = { i:l for i,l in enumerate(range(self.num_classes)) }
        elif dataset_name == 'SLAnimals_OF_3s':
            self.data_folder = './datasets//SL_Animals_OF/SL_animal_splits/dataset_3sets_12000/'
            self.width, self.height = 128, 128
            self.num_classes = 19
            self.class_mapping = { i:l for i,l in enumerate(range(self.num_classes)) }
        elif dataset_name == 'SLAnimals_SME_4s':
            self.data_folder = './datasets//SL_Animals_VM/SL_animal_splits/dataset_4sets_12000/'
            self.width, self.height = 128, 128
            self.num_classes = 19
            self.class_mapping = { i:l for i,l in enumerate(range(self.num_classes)) }
        elif dataset_name == 'SLAnimals_OF_4s':
            self.data_folder = './datasets//SL_Animals_OF/SL_animal_splits/dataset_4sets_12000/'
            self.width, self.height = 128, 128
            self.num_classes = 19
            self.class_mapping = { i:l for i,l in enumerate(range(self.num_classes)) }
            ...
            self.class_mapping = { i:l for i,l in enumerate(range(self.num_classes)) }
        elif dataset_name == 'fall_detection_dataset':
            self.data_folder = './datasets//Fall Detection Dataset/Fall_Detection_splits/dataset_4sets_12000/'
            self.width, self.height = 260, 346
            self.num_classes = 4          
            self.class_mapping = { i:l for i,l in enumerate('1 2 3 4'.split()) }
        elif dataset_name == 'FDD_OF':
            self.data_folder = './datasets/FDD_OF/FDD_OF_splits/dataset_4sets_12000/'
            self.width, self.height = 260, 346
            self.num_classes = 4          
            self.class_mapping = { i:l for i,l in enumerate('1 2 3 4'.split()) }
        elif dataset_name == 'FDD_SME':
            self.data_folder = './datasets//FDD_VM/FDD_SME_splits/dataset_4sets_12000/'
            self.width, self.height = 260, 346
            self.num_classes = 4          
            self.class_mapping = { i:l for i,l in enumerate('1 2 3 4'.split()) }
        elif dataset_name == 'ucf11':
            self.data_folder = './datasets/UCF11/UCF11_splits/dataset_11sets_12000/'
            self.width, self.height = 240, 240
            self.num_classes = 11          
            self.class_mapping = { i:l for i,l in enumerate(range(self.num_classes)) }
        elif dataset_name == 'ucf11_SME':
            self.data_folder = './datasets/UCF11_VM/UCF11_SME_splits/dataset_11sets_48000/'
            self.width, self.height = 240, 240
            self.num_classes = 11          
            self.class_mapping = { i:l for i,l in enumerate(range(self.num_classes)) } 
        elif dataset_name == 'ucf11_OF':
            self.data_folder = './datasets/UCF11_OF/UCF11_OF_splits/dataset_11sets_24000/'
            self.width, self.height = 240, 180
            self.num_classes = 11          
            self.class_mapping = { i:l for i,l in enumerate(range(self.num_classes)) }            
        elif dataset_name == 'IITM':
            self.data_folder = './datasets/IITM/IITM_splits/dataset_10sets_12000/'
            self.width, self.height = 128, 128
            self.num_classes = 10
            self.class_mapping = { i:l for i,l in enumerate(range(self.num_classes)) }
        elif dataset_name == 'IITM_SME':
            self.data_folder = './datasets/IITM_VM/IITM_SME_splits/dataset_10sets_12000/'
            self.width, self.height = 128, 128
            self.num_classes = 10
            self.class_mapping = { i:l for i,l in enumerate(range(self.num_classes)) }
        elif dataset_name == 'IITM_OF':
            self.data_folder = './datasets/IITM_OF/IITM_OF_splits/dataset_10sets_12000/'
            self.width, self.height = 128, 128
            self.num_classes = 10
            self.class_mapping = { i:l for i,l in enumerate(range(self.num_classes)) }
