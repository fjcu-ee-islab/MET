...
%%% Find the save directory of Result %%%
qname = '../Visual_motion';
pname = './Results';
dname = varargin{1}.filename;
savefolder = fullfile(pname,dname);

layer = varargin{1}.layer;
savefolder = fullfile(savefolder,layer);

if ~exist(savefolder, 'dir')
    mkdir(savefolder)
end

if ~strcmp(pwd, savefolder)
    cd(savefolder)
end
...
while (i<Tmax)
    ...
    imwrite(Image,sprintf('%d.tiff',i))
    frame = getframe(gcf);
