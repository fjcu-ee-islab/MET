...
from models.MEvT import CLFBlock, MLPBlock
from models.MEvT import EvNetBackbone
...
class EvNetModel(LightningModule):
    ...
    def forward(self, polarity, pixels):    
        # Get updated latent vectors
        embs = self.backbone(polarity, pixels)
    ...
    def step(self, polarity, pixels, y):
        embs, clf_logits = self(polarity, pixels)	# self.forward
    ...
    def training_step(self, batch, batch_idx):
        ...
        # batch_data -> (#timesteps, batch_size, #events, 2) - (#timesteps, batch_size, #events, 2) - (batch_size)
        polarity, pixels, y = batch
        losses = self.step(polarity, pixels, y)
        
        # pols, pols_pixels, pols_labels, sp, sp_pixels, sp_labels, dir, dir_pixels, dir_labels = batch
        # losses = self.step_in_train(pols, pols_pixels, pols_labels, sp, sp_pixels, sp_labels, dir, dir_pixels, dir_labels)
